...
Hyperband — bandit-метод successive halving: запускаем много конфигов на малом
бюджете, отсеиваем слабые, продолжаем сильные на большем бюджете.

Реализация: RandomSampler + HyperbandPruner из Optuna. Это honest baseline для
multi-fidelity: не использует BO, только successive halving.
"""
...
def train_adam_hyperband(loaders, meta, seed):
    set_seed(seed)
    cfg = CONFIG["adam_hyperband"]
    ...
    # n_trials рассчитывается достаточно щедро — pruner сам отсечёт лишнее
    ...
    for _ in tqdm(range(cfg["final_epochs"]), desc="HB final", leave=False):
